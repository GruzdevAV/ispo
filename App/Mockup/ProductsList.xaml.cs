...
namespace Mockup
{
    ...
    public partial class ProductsList : Window
    {
        ...
        public ProductsList(Window prevWindow)
        {
            InitializeComponent();
            _prevWindow = prevWindow;
            listview.ItemsSource = App.db.ListProducts.ToList();
            if (!new []{ "Администратор"}.Contains(App.User.Role.Name))
        ...
        private void tb_filter_TextChanged(object sender, TextChangedEventArgs e)
        {
            if (tb_filter.Text.Length>0)
                listview.ItemsSource = App.db.ListProducts
                    .Where(x => x.ProductName.ToUpper().Contains(tb_filter.Text.ToUpper()) ||
                                x.ShortName.ToUpper().Contains(tb_filter.Text.ToUpper()) ||
                                x.ProductArticleNumber.ToUpper().Contains(tb_filter.Text.ToUpper()) ||
